 ...
 * Layout:
 *   ┌─────────────────────────────────────────┐
 *   │  [‹]         Add Class           [✓]    │  ← control header
 *   ├─────────────────────────────────────────┤
 *   │  Subject      [________________]        │
 *   │  Teacher      [________________]        │
 *   │  Color        ⬤ ⬤ ⬤ ⬤ ⬤ ⬤ ⬤           │
 *   │  Class Time   [Day ▾] [Start ▾] [End ▾] │
 *   │               [+ Add Time]              │
 *   │  Classroom    [________________]        │
 *   └─────────────────────────────────────────┘
 *
 * Save flow:
 *   Collects form values → builds TimetableClassRecord → calls
 *   ctrl.saveClass(record) which appends it to the active timetable and
 *   fires activeTimetablePane back to "MAIN".
 */
public class TimetableAddClassPane extends VBox {
    ...
    /** Dynamic list of time-slot rows added by the user */
    ...
    private void onSave() {
        ...
        TimetableClassRecord rec = new TimetableClassRecord(
            subject, teacher, classroom, selectedColor);

        // Collect valid time slots
    ...
    private class TimeSlotRow {
        ...
        TimeSlotRow() {
            ...
            HBox rowNode = new HBox(6, dayBox, startBox, dash, endBox);
            ...
            remove.setOnAction(e -> {
                ...
                timeSlotsBox.getChildren().remove(rowNode);
            });

            rowNode.getChildren().add(remove);
            node = rowNode;
        ...
        private ComboBox<LocalTime> buildTimeCombo() {
            ...
            // 07:00 – 22:00 in 30-minute increments
            for (int h = 7; h <= 22; h++) {
